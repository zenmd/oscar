...
import org.oscarehr.PMmodule.model.caisi_Admission;
...
public class caisi_AdmissionDao extends HibernateDaoSupport {

    private Log log = LogFactory.getLog(caisi_AdmissionDao.class);
   ...
   public List getAdmissionsByFacility(Integer demographicNo, Integer facilityId) {
    ...
    public List getAdmissionList(Integer clientId, Integer facilityId, String providerNo) {
		...
		if (facilityId == 0) {
			...
			results = getHibernateTemplate().find("from Admission i where i.clientId = ? and i.programId in " + progSQL +
    ...
    public Integer getLastClientStatusFromAdmissionByProgramIdAndClientId(Integer programId, Integer demographicId) {
        ...
        caisi_Admission admission = null;
        ListIterator listIterator = results.listIterator();
        while (listIterator.hasNext()) {
            try {
                admission = (caisi_Admission) listIterator.next();
